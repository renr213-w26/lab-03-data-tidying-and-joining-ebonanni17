...
```
This data frame has 44 rows and 30 columns. Columns represent country and rows represent years. Each row indicates the inflation of each country in that specific year. Each column indicates the inflation of a single country for all the years listed. 
...
country_inflation %>%
  ...
  select(`2021`)
```
The inflation rates for Argentina, Türkiye, and Brazil are higher than the 
inflation rate in the United States in 2021.
...
country_inflation %>%
  ...
  print(n=Inf)
```
The inflation change of Brazil is the largest over this time period. Inflation 
decreased between 1993 and 2021 as indicated by the smallest inflation ratio 
(0.00431). A ratio less than 1 represents negative change.
...
inf_long<-country_inflation %>%
  ...
  inf_long %>%
    print(n=Inf)
```
This data frame has 3 columns and 1276 rows.

### Question 5

a.  What is the highest inflation rate observed between 1993 and 2021? In addition to code and output, your response should include a single sentence stating the country and year.

```{r}
inf_long%>%
  filter(annual_inflation==max(annual_inflation, na.rm = TRUE))
```
The highest inflation rate between 1993 and 2021 is observed in Brazil in 1994 
at approximately 2075.888. 

b.  What is the lowest inflation rate observed between 1993 and 2021? In addition to code and output, your response should include a single sentence stating the country and year.

```{r}
inf_long%>%
  filter(annual_inflation==min(annual_inflation, na.rm = TRUE))
```
The lowest inflation rate between 1993 and 2021 is observed in Ireland in 2009 
at approximately -4.478.
...
Picked 5 of the Nordic countries because I am curious about how inflation rates 
compare among them.
...
Denmark, Finland, Norway, and Sweden all exhibit similar inflation changes between the years of 1993 and 2021. Iceland, however, had sudden increases in annual inflation during years around 2000, as well as years around 2005, decreasing steadily just prior to 2010 back to levels similar to the other four nordic countries. 
